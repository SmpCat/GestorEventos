          ...
          {rules.map((rule: any, idx) => (
            ...
            >
              ...
              <div className="flex items-center justify-between pb-2" style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.1)' }}>
                ...
                {isAdmin && (
                  <button 
                    ...
                    title="Borrar Regla"
                    style={{ padding: '0.4rem 0.5rem', borderRadius: '6px', background: 'rgba(255, 255, 255, 0.1)', border: '1px solid rgba(255, 255, 255, 0.2)', color: '#ffffff' }}
                  >
                    <TrashIcon />
                  </button>
                )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full pt-3 mt-1" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                ...
                {/* Cuota a Cobrar (€) (Label claro arriba para que el símbolo o input no desborde a la derecha) */}
                <div className="w-full">
                  <label className="text-secondary text-xs font-bold block mb-1">Cuota a Cobrar (€)</label>
                  {isAdmin ? (
                    <input 
                      type="number" 
                      min="0" 
                      step="0.5"
                      placeholder="Ej. 25"
                      className="input-field w-full text-sm font-bold text-white text-left px-3"
                      style={{ padding: '0.45rem 0.75rem', boxSizing: 'border-box' }}
                      value={rule.price}
                      onChange={e => handleRuleChange(idx, 'price', e.target.value)}
                    />
                  ) : (
                    <strong className="text-white text-base block">{rule.price} €</strong>
                  )}
                </div>
